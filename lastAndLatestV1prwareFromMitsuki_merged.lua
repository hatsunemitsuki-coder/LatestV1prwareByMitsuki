...
-- ══════════════════════════════════════════════
--  SERVICES (local aliases for sentinel block)
-- ══════════════════════════════════════════════
local playersService    = svc.Players
local userInputService  = svc.Input
local runService        = svc.Run
local replicatedStorage = svc.RS
local workspaceService  = svc.WS
...
-- HDT concurrent-call guards
local hdtActive     = false
local hdtActiveConn = nil
...
local lineBlockDelay_lb       = cfg.get("lineBlockDelay",          0.05)
...
-- Block animation IDs — HDT triggers when local player's block anim plays
local BLOCK_ANIMS = {
...
local guestBlockAnimConnection                       = nil  -- HDT block-anim hook
...
-- Nearest killer helper (used by HDT block-anim trigger)
local function guestGetNearestKiller()
    local char = clientPlayer.Character; if not char then return nil end
...
local function showCinematicText()
    ...
    gui2.Name = "CinematicText"; gui2.Parent = PlayerGui
    gui2.IgnoreGuiInset = true; gui2.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    gui2.DisplayOrder = 10000
    local lbl = Instance.new("TextLabel")
    ...
    lbl.TextScaled = true; lbl.RichText = true; lbl.Parent = gui2
    cinematicGui = gui2
    task.delay(1.5, function()
        if cinematicGui then cinematicGui:Destroy(); cinematicGui = nil end
    end)
...
-- ══════════════════════════════════════════════
--  HITBOX DRAGGING (HDT)
--  Uses BodyVelocity for movement (like original HDT)
--  Triggers on BLOCK animations (like original HDT)
-- ══════════════════════════════════════════════
local function guestPerformHitboxDrag(killerModel)
    if not guestHitboxDraggingOn or not killerModel then return end
    if hdtActive then return end   -- guard: no concurrent calls
    ...
    if dragMode == "Hitbox" then
        ...
        if not targetPart then dragMode = "Blatant" end  -- graceful fallback
    end
    ...
    -- Initial mode resolution (Smart is re-evaluated per-frame below)
    local resolvedMode = dragMode
    if resolvedMode == "Smart" then
        local d = (targetPart.Position - myRoot.Position).Magnitude
        resolvedMode = (d <= hdtSwitchDist) and "Legit" or "Blatant"
    end

    -- ── Unified cleanup (runs exactly once regardless of exit path) ──
    local cleaned = false
    local function cleanup()
        if cleaned then return end
        cleaned = true
        hdtActive = false
        if hdtActiveConn then
            hdtActiveConn:Disconnect()
            hdtActiveConn = nil
        end
        pcall(function()
    ...
    if visual then
        ...
        debrisService:AddItem(beam, duration + 0.2)
    end

    local function updateBeam()
        if not beam or not beam.Parent then return end
        if not myRoot.Parent or not targetPart.Parent then return end
    ...
    -- BodyVelocity for movement (original HDT approach)
    ...
    hdtActiveConn = runService.Heartbeat:Connect(function()
        -- ── Stop conditions ──────────────────────────────────────────
        if tick() - startTime >= duration
        ...
        -- ── Re-evaluate Smart each frame ─────────────────────────────
        if dragMode == "Smart" then
            local d       = (targetPart.Position - myRoot.Position).Magnitude
            local newMode = (d <= hdtSwitchDist) and "Legit" or "Blatant"
            if newMode ~= resolvedMode then
        ...
        -- ── Movement via BodyVelocity ─────────────────────────────────
        local speed = (resolvedMode == "Blatant") and blatantSpeed or legitSpeed
        local to    = predPos - myRoot.Position
        local h2    = Vector3.new(to.X, 0, to.Z)
        if h2.Magnitude > 0.01 then
            bv.Velocity = h2.Unit * speed
        else
            bv.Velocity = Vector3.zero
        end
        ...
        -- ── Optional face-killer torque ───────────────────────────────
        if faceKiller then
            local fd = targetPart.Position - myRoot.Position
            if fd.Magnitude > 0.1 then
                local alpha = (resolvedMode == "Blatant") and 0.45 or 0.30
                myRoot.CFrame = myRoot.CFrame:Lerp(
                    CFrame.new(myRoot.Position, myRoot.Position + fd.Unit), alpha)
            end
        end
...
-- ── HDT Block-Anim trigger (fires when your block animation plays) ────
local _hdtLastAnimTime = 0
local HDT_ANIM_CD = 0.5

local function onBlockAnimPlayed(track)
    pcall(function()
        if not guestHitboxDraggingOn or hdtActive then return end
        local now = tick(); if now - _hdtLastAnimTime < HDT_ANIM_CD then return end
        ...
        _hdtLastAnimTime = now
        local nearest = guestGetNearestKiller()
        if nearest then
            task.spawn(guestPerformHitboxDrag, nearest)
        end
    end)
end
    ...
    task.spawn(function()
        ...
        for i = 1, count do
            ...
            if guestHRP and isPointInsidePart(part, guestHRP.Position) then
                ...
                if guestAutoPunchOn then task.delay(0.3, guestFireRemotePunch) end
                showCinematicText()
                break
            end
...
-- ══════════════════════════════════════════════
--  CHARACTER SETUP
--  Hooks punch aim AND block-anim HDT trigger
-- ══════════════════════════════════════════════
local function guestSetupCharacter(char)
    ...
    if guestBlockAnimConnection then guestBlockAnimConnection:Disconnect(); guestBlockAnimConnection = nil end
    local animator = guestHumanoid and guestHumanoid:FindFirstChildOfClass("Animator")
    if animator then
        if guestAimPunchActive then
        ...
        -- HDT block-anim hook on local player's animator
        guestBlockAnimConnection = animator.AnimationPlayed:Connect(onBlockAnimPlayed)
    end
end
            ...
            if kf and guestHRP then
                ...
                if targetHRP then
                    ...
                    local predictPos = targetHRP.Velocity.Magnitude > 0.5
                        and (targetHRP.Position + targetHRP.Velocity * (pred / 60))
    ...
    for _, killer in ipairs(kf:GetChildren()) do
        ...
        if not hrp then
            ...
            then
                ...
                task.delay(lineBlockDelay_lb, function()
...
clientPlayer.CharacterAdded:Connect(function(char)
    task.delay(0.5, function()
        -- Reset HDT state on respawn so it doesn't stay locked
        hdtActive = false
        if hdtActiveConn then hdtActiveConn:Disconnect(); hdtActiveConn = nil end
...
-- ══════════════════════════════════════════════════════════════════════
--  UI  (Rayfield — Sentinels tab)
-- ══════════════════════════════════════════════════════════════════════

-- ── SENTINEL SECTION ────────────────────────────────────────────────
tabSurSen:CreateSection("Auto Block & Combat")

tabSurSen:CreateToggle({ Name="Auto Block", CurrentValue=guestAutoBlockAudioOn, Flag="guestAutoBlockAudioOn",
    Callback=function(v) guestAutoBlockAudioOn=v; cfg.set("guestAutoBlockAudioOn",v) end })
tabSurSen:CreateSlider({ Name="Block Delay", Range={0,2}, Increment=0.05, CurrentValue=guestBlockDelay, Flag="guestBlockDelay",
    Callback=function(v) guestBlockDelay=v; cfg.set("guestBlockDelay",v) end })
tabSurSen:CreateSlider({ Name="Auto Block Radius", Range={1,50}, Increment=1, CurrentValue=guestDetectionRange, Flag="guestDetectionRange",
    Callback=function(v) guestDetectionRange=v; guestDetectionRangeSq=v*v; cfg.set("guestDetectionRange",v) end })
tabSurSen:CreateToggle({ Name="Verify Facing Check", CurrentValue=guestVerifyFacingCheckOn, Flag="guestVerifyFacingCheckOn",
    Callback=function(v) guestVerifyFacingCheckOn=v; cfg.set("guestVerifyFacingCheckOn",v) end })
tabSurSen:CreateToggle({ Name="Facing Check (new)", CurrentValue=guestVerifyFacingCheckOn, Flag="guestVerifyFacingCheckOnNew",
    Callback=function(v) guestVerifyFacingCheckOn=v; cfg.set("guestVerifyFacingCheckOn",v) end })
tabSurSen:CreateSlider({ Name="Vision Range", Range={1,20}, Increment=1, CurrentValue=guestVisionRange, Flag="guestVisionRange",
    Callback=function(v) guestVisionRange=v; cfg.set("guestVisionRange",v) end })
tabSurSen:CreateSlider({ Name="Vision Angle", Range={1,200}, Increment=1, CurrentValue=guestVisionAngle, Flag="guestVisionAngle",
    Callback=function(v) guestVisionAngle=v; cfg.set("guestVisionAngle",v) end })
tabSurSen:CreateToggle({ Name="Show Vision Cone", CurrentValue=guestShowVisionRange, Flag="guestShowVisionRange",
    Callback=function(v) guestShowVisionRange=v; cfg.set("guestShowVisionRange",v)
        if not v then guestCleanupVisionCone() end end })
tabSurSen:CreateToggle({ Name="Vision Cone (new)", CurrentValue=guestShowVisionRange, Flag="guestShowVisionRangeNew",
    Callback=function(v) guestShowVisionRange=v; cfg.set("guestShowVisionRange",v)
        if not v then guestCleanupVisionCone() end end })
tabSurSen:CreateToggle({ Name="No HDT Cooldown", CurrentValue=noHDTCooldown, Flag="noHDTCooldown",
    Callback=function(v) noHDTCooldown=v; cfg.set("noHDTCooldown",v) end })
tabSurSen:CreateToggle({ Name="Block Cooldown Checker", CurrentValue=hdtCooldownCheckerEnabled, Flag="hdtCooldownChecker",
...
tabSurSen:CreateToggle({ Name="Auto Punch", CurrentValue=guestAutoPunchOn, Flag="guestAutoPunchOn",
    Callback=function(v) guestAutoPunchOn=v; cfg.set("guestAutoPunchOn",v) end })
tabSurSen:CreateToggle({ Name="Aim Punch", CurrentValue=guestAimPunchActive, Flag="guestAimPunchActive",
...
tabSurSen:CreateSlider({ Name="Punch Prediction", Range={0,10}, Increment=1, CurrentValue=guestPunchPrediction, Flag="guestPunchPrediction",
    Callback=function(v) guestPunchPrediction=v; cfg.set("guestPunchPrediction",v) end })

tabSurSen:CreateSection("HDT (Hitbox Dragging)")
tabSurSen:CreateParagraph({ Title = "How HDT works", Content = "Activates on your block animation. Drags you toward the nearest killer using BodyVelocity. Supports Blatant, Legit, Hitbox, and Smart (auto-switch) modes." })
tabSurSen:CreateToggle({ Name="Enable HDT", CurrentValue=guestHitboxDraggingOn, Flag="guestHitboxDraggingOn",
    Callback=function(v) guestHitboxDraggingOn=v; cfg.set("guestHitboxDraggingOn",v) end })
tabSurSen:CreateDropdown({ Name="HDT Mode", Options={"Blatant","Legit","Hitbox","Smart"}, CurrentOption=guestHitboxMode, Flag="guestHitboxMode",
    Callback=function(v) guestHitboxMode=v; cfg.set("guestHitboxMode",v) end })
tabSurSen:CreateParagraph({ Title="Smart Mode", Content="Smart auto-switches between Legit (close) and Blatant (far) based on Switch Distance." })
tabSurSen:CreateSlider({ Name="Switch Distance (Smart)", Range={2,20}, Increment=1, CurrentValue=hdtSwitchDist, Flag="hdtSwitchDist",
    Callback=function(v) hdtSwitchDist=v; cfg.set("hdtSwitchDist",v) end })
tabSurSen:CreateSlider({ Name="Blatant Speed", Range={50,500}, Increment=5, CurrentValue=cfg.get("hdtBlatantSpeed",150), Flag="hdtBlatantSpeed",
    Callback=function(v) cfg.set("hdtBlatantSpeed",v) end })
tabSurSen:CreateSlider({ Name="Legit Speed", Range={50,500}, Increment=5, CurrentValue=cfg.get("hdtLegitSpeed",140), Flag="hdtLegitSpeed",
    Callback=function(v) cfg.set("hdtLegitSpeed",v) end })
tabSurSen:CreateSlider({ Name="Prediction Time (s)", Range={0,0.5}, Increment=0.01, CurrentValue=hdtPredictTime, Flag="hdtPredictTime",
    Callback=function(v) hdtPredictTime=v; cfg.set("hdtPredictTime",v) end })
tabSurSen:CreateToggle({ Name="Face Killer During HDT", CurrentValue=guestHitboxFaceKiller, Flag="guestHitboxFaceKiller",
    Callback=function(v) guestHitboxFaceKiller=v; cfg.set("guestHitboxFaceKiller",v) end })
tabSurSen:CreateToggle({ Name="HDT Visual (Beam)", CurrentValue=guestHitboxVisual, Flag="guestHitboxVisual",
    Callback=function(v) guestHitboxVisual=v; cfg.set("guestHitboxVisual",v) end })
tabSurSen:CreateSlider({ Name="Drag Duration", Range={0.05,2}, Increment=0.05, CurrentValue=guestHitboxDragDuration, Flag="guestHitboxDragDuration",
    Callback=function(v) guestHitboxDragDuration=v; cfg.set("guestHitboxDragDuration",v) end })
tabSurSen:CreateSlider({ Name="Trigger Radius", Range={0,30}, Increment=1, CurrentValue=guestHitboxTriggerRadius, Flag="guestHitboxTriggerRadius",
    Callback=function(v) guestHitboxTriggerRadius=v; cfg.set("guestHitboxTriggerRadius",v) end })
tabSurSen:CreateDropdown({ Name="Target Mode", Options={"Root","Arms","Random","Hitbox"}, CurrentOption=guestHitboxTargetMode, Flag="guestHitboxTargetMode",
    Callback=function(v) guestHitboxTargetMode=v; cfg.set("guestHitboxTargetMode",v) end })
tabSurSen:CreateToggle({ Name="Show HDT Range", CurrentValue=guestShowHDTRange, Flag="guestShowHDTRange",
    Callback=function(v) guestShowHDTRange=v; cfg.set("guestShowHDTRange",v) end })

tabSurSen:CreateSection("Better Detection")
tabSurSen:CreateToggle({ Name="Enable Better Detection", CurrentValue=guestBetterDetectionEnabled, Flag="guestBetterDetectionEnabled",
    Callback=function(v) guestBetterDetectionEnabled=v; cfg.set("guestBetterDetectionEnabled",v) end })
tabSurSen:CreateSlider({ Name="Hitbox X", Range={0.5,10}, Increment=0.5, CurrentValue=guestBetterDetectionX, Flag="guestBetterDetectionX",
    Callback=function(v) guestBetterDetectionX=v; cfg.set("guestBetterDetectionX",v) end })
tabSurSen:CreateSlider({ Name="Hitbox Y", Range={0.5,10}, Increment=0.5, CurrentValue=guestBetterDetectionY, Flag="guestBetterDetectionY",
    Callback=function(v) guestBetterDetectionY=v; cfg.set("guestBetterDetectionY",v) end })
tabSurSen:CreateSlider({ Name="Hitbox Z (forward)", Range={0.5,10}, Increment=0.5, CurrentValue=guestBetterDetectionZ, Flag="guestBetterDetectionZ",
    Callback=function(v) guestBetterDetectionZ=v; cfg.set("guestBetterDetectionZ",v) end })
tabSurSen:CreateSlider({ Name="Multiplier", Range={0.5,5}, Increment=0.1, CurrentValue=guestBetterDetectionMultiplier, Flag="guestBetterDetectionMultiplier",
    Callback=function(v) guestBetterDetectionMultiplier=v; cfg.set("guestBetterDetectionMultiplier",v) end })
tabSurSen:CreateSlider({ Name="Segments", Range={1,30}, Increment=1, CurrentValue=guestBetterDetectionSegments, Flag="guestBetterDetectionSegments",
    Callback=function(v) guestBetterDetectionSegments=math.max(1,math.floor(v)); cfg.set("guestBetterDetectionSegments",guestBetterDetectionSegments) end })
tabSurSen:CreateSlider({ Name="Start Distance", Range={0,10}, Increment=0.1, CurrentValue=guestBetterStartDist, Flag="guestBetterStartDist",
    Callback=function(v) guestBetterStartDist=v; cfg.set("guestBetterStartDist",v) end })
tabSurSen:CreateSlider({ Name="Step Distance", Range={0,0.5}, Increment=0.01, CurrentValue=guestBetterStepDist, Flag="guestBetterStepDist",
    Callback=function(v) guestBetterStepDist=v; cfg.set("guestBetterStepDist",v) end })
tabSurSen:CreateSlider({ Name="Pre-Delay (s)", Range={0,1}, Increment=0.05, CurrentValue=guestBetterPreDelay, Flag="guestBetterPreDelay",
    Callback=function(v) guestBetterPreDelay=v; cfg.set("guestBetterPreDelay",v) end })
tabSurSen:CreateSlider({ Name="Stagger (s)", Range={0,0.1}, Increment=0.005, CurrentValue=guestBetterStagger, Flag="guestBetterStagger",
    Callback=function(v) guestBetterStagger=v; cfg.set("guestBetterStagger",v) end })

tabSurSen:CreateSection("Lineblock")
tabSurSen:CreateToggle({ Name="Enable Lineblock", CurrentValue=lineBlockEnabled, Flag="lineBlockEnabled",
    Callback=function(v) lineBlockEnabled=v; cfg.set("lineBlockEnabled",v) end })
tabSurSen:CreateSlider({ Name="Line Length", Range={2,40}, Increment=1, CurrentValue=lineBlockLength, Flag="lineBlockLength",
    Callback=function(v) lineBlockLength=v; cfg.set("lineBlockLength",v) end })
tabSurSen:CreateSlider({ Name="Line Radius", Range={0.5,10}, Increment=0.5, CurrentValue=lineBlockRadius, Flag="lineBlockRadius",
    Callback=function(v) lineBlockRadius=v; cfg.set("lineBlockRadius",v) end })
tabSurSen:CreateSlider({ Name="Block Delay (x0.05s)", Range={0,20}, Increment=1, CurrentValue=math.floor(lineBlockDelay_lb/0.05+0.5), Flag="lineBlockDelay",
    Callback=function(v) lineBlockDelay_lb=v*0.05; cfg.set("lineBlockDelay",lineBlockDelay_lb) end })
tabSurSen:CreateSlider({ Name="Anti-Bait Hold (x0.01s)", Range={0,50}, Increment=1, CurrentValue=math.floor(lineBlockFacingDuration/0.01+0.5), Flag="lineBlockFacingDuration",
    Callback=function(v) lineBlockFacingDuration=v*0.01; cfg.set("lineBlockFacingDuration",lineBlockFacingDuration) end })
tabSurSen:CreateSlider({ Name="Facing Sensitivity", Range={1,10}, Increment=1, CurrentValue=math.floor(lineBlockMinDot*10+0.5), Flag="lineBlockMinDot",
    Callback=function(v) lineBlockMinDot=v/10; cfg.set("lineBlockMinDot",lineBlockMinDot) end })
tabSurSen:CreateParagraph({ Title="Color Guide", Content="White = idle  |  Yellow = punch anim or sound detected  |  Red = you're on the line (block fires)" })

tabSurSen:CreateSection("Punch Mobility")
tabSurSen:CreateToggle({ Name="Enable Punch Mobility", CurrentValue=punchMobilityEnabled, Flag="punchMobilityEnabled",
...
tabSurSen:CreateToggle({ Name="Lunge Boost", CurrentValue=lungeBoostEnabled, Flag="lungeBoostEnabled",
...
tabSurSen:CreateSlider({ Name="Lunge Multiplier", Range={0.5,5}, Increment=0.1, CurrentValue=lungeMultiplier, Flag="lungeMultiplier",
    Callback=function(v) lungeMultiplier=v; cfg.set("lungeMultiplier",v) end })
tabSurSen:CreateToggle({ Name="Charge Control", CurrentValue=chargeControlEnabled, Flag="chargeControlEnabled",
    Callback=function(v) chargeControlEnabled=v; cfg.set("chargeControlEnabled",v)
        if not v then stopLock() end end })
tabSurSen:CreateSlider({ Name="Charge Speed Limit", Range={1,50}, Increment=1, CurrentValue=chargeSpeedLimit, Flag="chargeSpeedLimit",
    Callback=function(v) chargeSpeedLimit=v; cfg.set("chargeSpeedLimit",v) end })
tabSurSen:CreateToggle({ Name="Charge Speed Cap", CurrentValue=chargeSpeedCapEnabled, Flag="chargeSpeedCapEnabled",
...
tabSurSen:CreateToggle({ Name="Reverse Block", CurrentValue=guestReverseEnabled, Flag="guestReverseEnabled",
    Callback=function(v) guestReverseEnabled=v; cfg.set("guestReverseEnabled",v) end })
tabSurSen:CreateToggle({ Name="Legacy Facing", CurrentValue=guestLegacyFacingEnabled, Flag="guestLegacyFacingEnabled",
    Callback=function(v) guestLegacyFacingEnabled=v; cfg.set("guestLegacyFacingEnabled",v) end })
tabSurSen:CreateSlider({ Name="Legacy Vision Range", Range={1,20}, Increment=1, CurrentValue=guestLegacyVisionRange, Flag="guestLegacyVisionRange",
    Callback=function(v) guestLegacyVisionRange=v; cfg.set("guestLegacyVisionRange",v) end })
tabSurSen:CreateSlider({ Name="Legacy Vision Angle", Range={1,200}, Increment=1, CurrentValue=guestLegacyVisionAngle, Flag="guestLegacyVisionAngle",
    Callback=function(v) guestLegacyVisionAngle=v; cfg.set("guestLegacyVisionAngle",v) end })
tabSurSen:CreateToggle({ Name="Show Legacy Vision Range", CurrentValue=guestLegacyShowVisionRange, Flag="guestLegacyShowVisionRange",
    Callback=function(v) guestLegacyShowVisionRange=v; cfg.set("guestLegacyShowVisionRange",v) end })

tabSurSen:CreateSection("Misc")
tabSurSen:CreateToggle({ Name="Cinematic Text", CurrentValue=guestCinematicTextEnabled, Flag="guestCinematicTextEnabled",
    Callback=function(v) guestCinematicTextEnabled=v; cfg.set("guestCinematicTextEnabled",v) end })
tabSurSen:CreateToggle({ Name="Smart Delay", CurrentValue=guestSmartDelayEnabled, Flag="guestSmartDelayEnabled",
    Callback=function(v) guestSmartDelayEnabled=v; cfg.set("guestSmartDelayEnabled",v) end })
tabSurSen:CreateToggle({ Name="Manual Drag", CurrentValue=guestManualDragEnabled, Flag="guestManualDragEnabled",
    Callback=function(v) guestManualDragEnabled=v; cfg.set("guestManualDragEnabled",v) end })
tabSurSen:CreateToggle({ Name="Noclip", CurrentValue=noclipEnabled, Flag="noclipEnabled",
    Callback=function(v) noclipEnabled=v; cfg.set("noclipEnabled",v)
        if v then enableNoclip() else disableNoclip() end end })
tabSurSen:CreateToggle({ Name="Noclip Only When Block Ready", CurrentValue=noclipOnlyWhenBlockReady, Flag="noclipOnlyWhenBlockReady",
    Callback=function(v) noclipOnlyWhenBlockReady=v; cfg.set("noclipOnlyWhenBlockReady",v) end })
tabSurSen:CreateToggle({ Name="FPS Counter", CurrentValue=fpsCounterEnabled, Flag="fpsCounterEnabled",
...
tabSurSen:CreateToggle({ Name="Ping Checker", CurrentValue=pingCheckerEnabled, Flag="pingCheckerEnabled",
