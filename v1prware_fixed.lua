...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabSettings = win:CreateTab("Settings", 4483362458)
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabGlobal  = win:CreateTab("Global", 4483362458)
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabGen = win:CreateTab("Generator", 4483362458)
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabKiller = win:CreateTab("Killer", 4483362458)
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabVisual = win:CreateTab("Visual", 4483362458)
...
local espColors = {
    killer    = loadColor("espColKiller",   139,   0,   0),
    survivor  = loadColor("espColSurvivor",  34, 139,  34),
    generator = loadColor("espColGen",      255, 105, 180),
    medkit    = loadColor("espColMedkit",     0, 255, 255),
    bloxycola = loadColor("espColBloxy",    255, 165,   0),
...
local function espItemColor(name)
    local n = name:lower()
    if n:find("medkit")    then return espColors.medkit    end
    if n:find("bloxycola") then return espColors.bloxycola end
...
tabVisual:CreateColorPicker({ Name = "Killer Color", Color = espColors.killer, Flag = "espColKiller",
    Callback = function(col) espColors.killer = col; saveColor("espColKiller", col); if esp.killers then espDoKillers(false); espDoKillers(true) end end })
tabVisual:CreateColorPicker({ Name = "Survivor Color", Color = espColors.survivor, Flag = "espColSurvivor",
    Callback = function(col) espColors.survivor = col; saveColor("espColSurvivor", col); if esp.survivors then espDoSurvivors(false); espDoSurvivors(true) end end })
tabVisual:CreateColorPicker({ Name = "Generator Color", Color = espColors.generator, Flag = "espColGen",
    Callback = function(col) espColors.generator = col; saveColor("espColGen", col); if esp.generators then espDoGenerators(false); espDoGenerators(true) end end })
tabVisual:CreateColorPicker({ Name = "Item Color", Color = espColors.medkit, Flag = "espColItem",
    Callback = function(col) espColors.medkit = col; espColors.bloxycola = col; saveColor("espColMedkit", col); saveColor("espColBloxy", col); if esp.items then espDoItems(false); espDoItems(true) end end })
tabVisual:CreateColorPicker({ Name = "Building Color", Color = espColors.building, Flag = "espColBuilding",
    Callback = function(col) espColors.building = col; saveColor("espColBuilding", col); if esp.buildings then espDoBuildings(false); espDoBuildings(true) end end })
...
tabVisual:CreateParagraph({ Title = "How it works", Content = "Tracks c00lkidd's pizza bots (orange), 1x1x1x1 zombies (green), and John Doe's shadow puddles (black disc with red glow)." })
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabMusic = win:CreateTab("Music", 4483362458)
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabSurSen = win:CreateTab("Sentinels", 4483362458)
...
local combatS = {
    ...
    hdtMode = cfg.get("combatHDTMode", "180_TURN"),
    hdtRotateDelay = cfg.get("combatHDTRotateDelay", 0),
...
local TRIGGER_SOUNDS = {
    ...
    ["124397369810639"] = true, ["76467993976301"] = true, ["118493324723683"] = true, ["78298577002481"] = true,
    ["116527305931161"] = true, ["5148302439"] = true, ["98675142200448"] = true, ["128367348686124"] = true,
    ["71805956520207"] = true, ["125213046326879"] = true, ["84353899757208"] = true,
    ["103684883268194"] = true, ["109246041199659"] = true, ["80540530406270"] = true,
    ["139523195429581"] = true, ["105204810054381"] = true,
}
...
local function combatRegisterSound(sound)
    ...
    local playedConn = sound.Played:Connect(function()
        if combatS.autoBlockOn then task.spawn(combatTryBlock, sound) end
    end)
    local propConn = sound:GetPropertyChangedSignal("IsPlaying"):Connect(function()
        if sound.IsPlaying and combatS.autoBlockOn then task.spawn(combatTryBlock, sound) end
    end)
    sound.Destroying:Connect(function()
        pcall(function() playedConn:Disconnect(); propConn:Disconnect() end)
...
local function combatSetupSoundWatcher()
    ...
    kf.ChildRemoved:Connect(function()
        task.wait(0.1)
...
do
    ...
    local function onBlockAnimPlayed(track)
        pcall(function()
            if not combatS.hdtEnabled or _debounce then return end
...
local function combatRefreshAnimator()
    ...
    combatHDT.setup(_cachedAnimator)
end
...
tabSurSen:CreateToggle({ Name="Auto Block (Audio)", CurrentValue=combatS.autoBlockOn, Flag="combatABOn",
    Callback=function(on) combatS.autoBlockOn=on; cfg.set("combatABOn",on); if on then combatSetupSoundWatcher(); combatStartLoops() else combatStopLoops() end end })
...
tabSurSen:CreateParagraph({ Title = "How HDT works", Content = "Activates the moment you block (detected via block animation). Drags you toward the nearest killer at high speed. Choose turn mode: 180 Turn, Left Spin, or Right Spin." })
...
tabSurSen:CreateDropdown({ Name="HDT Mode", Options={"180_TURN","LEFT_SPIN","RIGHT_SPIN"}, CurrentOption=combatS.hdtMode, Flag="combatHDTMode", Callback=function(v) combatS.hdtMode=v; cfg.set("combatHDTMode",v) end })
...
tabSurSen:CreateSlider({ Name="HDT Rotate Delay (s)", Range={0,0.5}, Increment=0.01, CurrentValue=combatS.hdtRotateDelay, Flag="combatHDTRotateDelay", Callback=function(v) combatS.hdtRotateDelay=v; cfg.set("combatHDTRotateDelay",v) end })
...
tabSurSen:CreateSlider({ Name="Facing Visual Size", Range={1,10}, Increment=0.5, CurrentValue=combatS.facingVisRadius, Flag="combatFacingVisRadius", Callback=function(v) combatS.facingVisRadius=v; cfg.set("combatFacingVisRadius",v); for _, vis in pairs(combatFacingVisuals) do if vis and vis.Parent then vis.Radius = v; local adornee = vis.Adornee; if adornee then vis.CFrame = CFrame.new(0, -(adornee.Size.Y / 2 + 0.04), -v) * CFrame.Angles(math.rad(90), 0, 0) end end end end })

tabSurSen:CreateSection("Character Lock")
tabSurSen:CreateToggle({ Name="Lock On Punch", CurrentValue=combatS.charLockOn, Flag="combatCharLock", Callback=function(on) combatS.charLockOn=on; cfg.set("combatCharLock",on) end })
tabSurSen:CreateSlider({ Name="Lock Max Distance", Range={5,100}, Increment=5, CurrentValue=combatS.lockMaxDist, Flag="combatLockMaxDist", Callback=function(v) combatS.lockMaxDist=v; cfg.set("combatLockMaxDist",v) end })
tabSurSen:CreateSlider({ Name="Prediction", Range={0,15}, Increment=0.5, CurrentValue=combatS.predictionVal, Flag="combatPrediction", Callback=function(v) combatS.predictionVal=v; cfg.set("combatPrediction",v) end })
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabVeeronica = win:CreateTab("Veeronica", 4483362458)
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabJaneDoe = win:CreateTab("Jane Doe", 4483362458)
...
do
    ...
    tabJaneDoe:CreateParagraph({
        Title   = "How it works",
        Content = "Patches Jane Doe's RemoteFunction so every crystal throw auto-fires on a loop with silent aim.",
    })

    tabJaneDoe:CreateToggle({
        Name = "Enable Auto-Fire", CurrentValue = false,
        Callback = function(on)
            jd_enabled = on
    ...
    tabJaneDoe:CreateToggle({
        Name = "Aimbot (Silent Aim)", CurrentValue = false,
        Callback = function(on)
            jd_aimbotOn = on
    ...
    tabJaneDoe:CreateSlider({
        Name = "Aim Offset (Y)", Range = { -5.0, 5.0 }, Increment = 0.1,
        CurrentValue = jd_AIM_OFFSET,
        Callback = function(v) jd_AIM_OFFSET = v end
    })

    tabJaneDoe:CreateSlider({
        Name = "Prediction", Range = { 0.0, 1.0 }, Increment = 0.01,
        CurrentValue = jd_PREDICTION,
        Callback = function(v) jd_PREDICTION = v end
    })

    tabJaneDoe:CreateSlider({
        Name = "Hold Duration (s)", Range = { 0.3, 2.0 }, Increment = 0.1,
    ...
    tabJaneDoe:CreateParagraph({
        Title   = "How it works",
        Content = "Intercepts your axe FireServer call and locks your character facing the nearest killer for the set duration. Camera never moves.",
    })

    tabJaneDoe:CreateToggle({
        Name = "Enable Axe Lock", CurrentValue = false,
        Callback = function(on)
    ...
    tabJaneDoe:CreateSlider({
        Name = "Lock Duration (s)", Range = { 0.5, 3.0 }, Increment = 0.1,
...
------------------------------------------------------------------------
------------------------------------------------------------------------
local tabNoli = win:CreateTab("Noli", 4483362458)
...
-- Interface Tab
------------------------------------------------------------------------
local tabInterface = win:CreateTab("Interface", 4483362458)
