...
local function combatRegisterSound(sound)
    ...
    -- Hook both Played and IsPlaying property change for maximum reliability
    ...
    -- Fire immediately if already playing when hooked
    if sound.IsPlaying and combatS.autoBlockOn then task.spawn(combatTryBlock, sound) end
end
...
local function combatOnBlockAnim(track) end -- legacy stub, unused
...
tabSurSen:CreateDropdown({ Name="HDT Mode", Options={"180_TURN","LEFT_SPIN","RIGHT_SPIN"}, CurrentOption=combatS.hdtMode, Flag="combatHDTMode",
    Callback=function(v) combatS.hdtMode=v; cfg.set("combatHDTMode",v) end
})
...
tabSurSen:CreateSlider({ Name="HDT Rotate Delay (s)", Range={0,0.5}, Increment=0.01, CurrentValue=combatS.hdtRotateDelay, Flag="combatHDTRotateDelay",
    Callback=function(v) combatS.hdtRotateDelay=v; cfg.set("combatHDTRotateDelay",v) end 
})
